 ...
 * realloc() can be used to increase or decrease the size of a pre-existing memory block.
 ...
 * i. pointer pointing to old memory location
 * ii. the size of the new memory block to be allocated.
 *
 * If the pointer given as argument is NULL then malloc() is used to create a momory allocation
 ...
 * 
 * If the memory is no sufficient in the old_block then the next block is checked if its free
 * and fused if possible.
 *
 * If the memory formed is more than requried after fusing with the next block,
 * then the extra memory is split into a new block.
 *
 * Memory is allocated newly and the data is copied if enough memory is not found
 * even after fusing the next free block(if available).
 * 
...
void *realloc(void *p, size_t size)
{
		 ...
		 *
        	 * old_block->size returns the size of the old memory block passed as an argument 
		 * when realloc() is invoked.
		 *
        	 * META_BLOCK_SIZE is the size of the block storing the meta information 
